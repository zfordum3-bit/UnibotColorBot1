...
class Utils:
    ...
    def check_key_binds(self):  # Return a boolean based on if the config needs to be reloaded
        if self.key_reload_config != 'off' and win32api.GetAsyncKeyState(self.key_reload_config) < 0:
            return True

        # Keep reload explicit. Editors, sync tools, and some file watchers can
        # touch config.ini during heavy runs, which used to restart serial/capture.

        if self.key_toggle_aim != 'off' and win32api.GetAsyncKeyState(self.key_toggle_aim) < 0:
            self.aim_state = not self.aim_state
            print("AIM: " + str(self.aim_state))
            sleep(self.delay)

        if self.key_toggle_recoil != 'off' and win32api.GetAsyncKeyState(self.key_toggle_recoil) < 0:
            self.recoil_state = not self.recoil_state
            print("RECOIL: " + str(self.recoil_state))
            sleep(self.delay)
        
        if self.key_exit != 'off' and win32api.GetAsyncKeyState(self.key_exit) < 0:
            print("Exiting")
            exit(1)
        return False
    ...
    def get_config_mtime(self):
        try:
            return os.path.getmtime(self.config.path)
        except OSError:
            return None
    ...
    def config_file_changed(self):
        ...
        if current_mtime is not None and current_mtime != self.config_mtime:
            self.config_mtime = current_mtime
            print('Config changed on disk')
            return True
    ...
    def get_trigger_state(self):
        if self.key_trigger != 'off' and win32api.GetAsyncKeyState(self.key_trigger) < 0:
            return True
        return False

    def get_rapid_fire_state(self):
        if self.key_rapid_fire != 'off' and win32api.GetAsyncKeyState(self.key_rapid_fire) < 0:
            return True
        return False
